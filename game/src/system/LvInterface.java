...
public interface LvInterface {
    ...
    public static void randomizeLevel(Level level) {
        ...
        for (int i = 0; i < numBarriers; i++) {
            switch (barrierType) {
                default -> addBarrier(level, new SimpleBarrier(new Point(100 * i, 100 * i), new Dimension(51, 20))); // %40
                                                                                                                     // chance
                case 4, 5, 6, 7, 8, 9, 10 ->
                    addBarrier(level, new ReinforcedBarrier(new Point(100 * i, 100 * i), new Dimension(51, 20))); // %30
                                                                                                                  // chance
                case 0, 1, 2, 3 ->
                    addBarrier(level, new ExplosiveBarrier(new Point(100 * i, 100 * i), new Dimension(51, 20))); // %20
                                                                                                                 // chance
                case 19 -> addBarrier(level, new RewardingBarrier(new Point(100 * i, 100 * i), new Dimension(51, 20))); // %5
                                                                                                                        // chance
